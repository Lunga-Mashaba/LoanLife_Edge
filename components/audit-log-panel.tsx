    ...
    return (
      <Card className="p-4 sm:p-6 bg-card border-border">
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <h3 className="text-lg sm:text-xl font-semibold text-card-foreground">Audit Log</h3>
  ...
  if (error) {
    return (
      <Card className="p-4 sm:p-6 bg-card border-border">
        ...
        <div className="p-3 sm:p-4 rounded-lg bg-muted border border-[oklch(0.55_0.20_25)]">
          ...
          <>
            {displayLogs.map((log) => {
              ...
              return (
                <div
                  key={log.id}
                  className="p-2.5 sm:p-3 rounded-lg bg-muted border border-border hover:border-[oklch(0.50_0.22_290)] transition-all duration-200"
                >
                  <div className="flex items-start gap-2 sm:gap-3">
                    <Icon className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[oklch(0.60_0.22_290)] mt-0.5 flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs sm:text-sm font-medium text-card-foreground break-words">{formatEventType(log.event_type)}</p>
                      <p className="text-[10px] sm:text-xs text-muted-foreground mt-1 break-words">{log.description}</p>
                      {log.loan_id && (
                        <p className="text-[10px] sm:text-xs text-muted-foreground mt-1 font-mono truncate">{log.loan_id}</p>
                      )}
                      {txHash && (
                        <p className="text-[10px] sm:text-xs text-[oklch(0.50_0.22_290)] font-mono mt-1 truncate">{formatTxHash(txHash)}</p>
                      )}
                    </div>
                    <span className="text-[10px] sm:text-xs text-muted-foreground whitespace-nowrap flex-shrink-0">{timeAgo}</span>
                  </div>
                </div>
              )
            })}
              ...
              <div className="pt-2 border-t border-border">
                {!showAll ? (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <p className="text-xs sm:text-sm text-muted-foreground">Showing all {logs.length} logs</p>
                    <Link
                      href="/audit-log"

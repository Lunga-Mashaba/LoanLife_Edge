        ...
        <main className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 pl-12 md:pl-3">
          ...
          <div className="space-y-3 max-w-4xl">
            {notifications.map((notification) => {
              ...
              let iconColor = "text-[oklch(0.60_0.02_250)]"
              let bgColor = "bg-[oklch(0.14_0.02_250)]"
              let borderColor = "border-[oklch(0.25_0.04_250)]"

              if (notification.type === "critical") {
                iconColor = "text-[oklch(0.55_0.20_25)]"
                bgColor = notification.read ? "bg-[oklch(0.14_0.02_250)]" : "bg-[oklch(0.16_0.03_25)]/30"
                borderColor = "border-[oklch(0.35_0.15_25)]"
              } else if (notification.type === "warning") {
                iconColor = "text-[oklch(0.75_0.18_65)]"
                bgColor = notification.read ? "bg-[oklch(0.14_0.02_250)]" : "bg-[oklch(0.16_0.03_65)]/20"
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className={`p-2 rounded-lg bg-[oklch(0.10_0.02_250)]`}>
                      <Icon className={`h-5 w-5 ${iconColor}`} />
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className="font-semibold text-[oklch(0.95_0.01_250)]">{notification.title}</h3>
                        {!notification.read && (
                          <div className="h-2 w-2 rounded-full bg-[oklch(0.55_0.20_220)] animate-pulse" />
                        )}
                      </div>
                      <p className="text-sm text-[oklch(0.70_0.02_250)] mb-2">{notification.message}</p>
                      <span className="text-xs text-[oklch(0.50_0.02_250)] font-mono">{notification.time}</span>
                    </div>
                  </div>
                </Card>
              )
          ...
          {notifications.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16">
              <Bell className="h-16 w-16 text-[oklch(0.30_0.04_250)] mb-4" />
              <h3 className="text-lg font-semibold text-[oklch(0.70_0.02_250)] mb-2">No notifications</h3>
              <p className="text-sm text-[oklch(0.50_0.02_250)]">You're all caught up!</p>
            </div>
          )}
        </main>
      </div>

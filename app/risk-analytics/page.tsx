      ...
      <div className="flex flex-1 flex-col min-w-0 w-full md:pl-64">
        ...
        <main className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6">
          <div className="mb-4 sm:mb-6">
            <h1 className="text-xl sm:text-2xl font-bold text-foreground mb-1 sm:mb-2">Risk Analytics</h1>
            <p className="text-sm sm:text-base text-muted-foreground">AI-powered predictive risk assessment</p>
          </div>

          {/* Risk Metrics Grid */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
            {riskMetrics.map((metric, idx) => (
              <Card key={idx} className="bg-card border-border p-3 sm:p-4">
                <div className="text-[10px] sm:text-xs text-muted-foreground mb-1 sm:mb-2">{metric.label}</div>
          ...
          <Card className="bg-card border-border p-4 sm:p-6 mb-4 sm:mb-6">
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              ...
              <div className="min-w-0">
                <h2 className="text-base sm:text-lg font-semibold text-card-foreground">AI Breach Predictions</h2>
                <p className="text-xs sm:text-sm text-muted-foreground">ML-powered covenant breach forecasting</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
              {predictions.map((pred, idx) => (
                <div
                  key={idx}
                  className="bg-muted rounded-lg p-3 sm:p-4 border border-border hover:border-[oklch(0.35_0.08_250)] transition-all duration-300 hover:transform hover:scale-105 cursor-pointer"
                >
                  <div className="flex items-center justify-between mb-2 sm:mb-3">
                    <div className="text-xs sm:text-sm font-semibold text-card-foreground">{pred.period}</div>
                  ...
                  <div className="text-[10px] sm:text-xs text-muted-foreground mb-2 sm:mb-3">Breach Probability</div>
                  <div className="flex items-center justify-between text-[10px] sm:text-xs">
                    <span className="text-muted-foreground">Confidence: {pred.confidence}</span>
          ...
          <Card className="bg-card border-border p-4 sm:p-6">
            <h2 className="text-base sm:text-lg font-semibold text-card-foreground mb-3 sm:mb-4">Top Risk Factors</h2>
                ...
                <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1 gap-2">
                      <span className="text-xs sm:text-sm font-medium text-card-foreground truncate">{risk.factor}</span>
                      <span className="text-xs sm:text-sm font-mono text-muted-foreground flex-shrink-0">{risk.score}</span>
                    </div>
